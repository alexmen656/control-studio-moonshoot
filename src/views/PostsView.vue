...
const loadposts = async () => {
    try {
        ...
        if (response.ok) {
            ...
            posts.value = data.map((v: any) => ({
                ...v,
                uploadDate: new Date(v.uploadDate)
            }))

            posts.value.forEach(async (video) => {
...
const updateVideoDuration = async (video: Video) => {
    try {
        ...
        videoElement.onloadedmetadata = async () => {
            ...
            const index = posts.value.findIndex(v => v.id === video.id)
            if (index !== -1 && posts.value[index]) {
                posts.value[index].duration = formattedDuration
            }

            await fetch(`${API_URL}/posts/${video.id}/duration`, {
                method: 'PATCH',
    ...
    for (const file of files) {
        try {
            ...
            if (response.ok) {
                ...
                const index = posts.value.findIndex(v => v.id === tempId)
                if (index !== -1) {
                    const updatedVideo = {
                        ...result.video,
                    ...
                    posts.value[index] = updatedVideo
                    updateVideoDuration(updatedVideo)
                }
            } else {
                const index = posts.value.findIndex(v => v.id === tempId)
                if (index !== -1 && posts.value[index]) {
                    posts.value[index].status = 'failed'
                }
            }
        } catch (error) {
...
const deleteVideo = async (id: string) => {
    ...
    try {
        ...
        if (response.ok) {
            posts.value = posts.value.filter(v => v.id !== id)
        }
    } catch (error) {
...
const bulkDelete = async () => {
    if (!confirm(`Delete ${selectedposts.value.size} selected posts?`)) return

    try {
        const response = await fetch(`${API_URL}/posts/bulk-delete`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ videoIds: Array.from(selectedposts.value) })
        })

        if (response.ok) {
            posts.value = posts.value.filter(v => !selectedposts.value.has(v.id))
...
const filteredposts = computed(() => {
    return posts.value.filter(video => {
...
const selectAllposts = () => {
    if (selectedposts.value.size === filteredposts.value.length) {
        selectedposts.value.clear()
    } else {
        selectedposts.value = new Set(filteredposts.value.map(v => v.id))
    }
}

const togglepostselection = (id: string) => {
    if (selectedposts.value.has(id)) {
        selectedposts.value.delete(id)
    } else {
        selectedposts.value.add(id)
    }
}
...
const bulkSchedule = () => {
    alert(`Schedule ${selectedposts.value.size} posts`)
}
...
const saveVideoDetails = async () => {
    ...
    try {
        const response = await fetch(`${API_URL}/posts/${selectedVideoForDetails.value.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (response.ok) {
            const result = await response.json()
            const index = posts.value.findIndex(v => v.id === selectedVideoForDetails.value!.id)
            if (index !== -1) {
                posts.value[index] = {
                    ...posts.value[index],
                    ...result.video,
...
<template>
    <div class="h-full flex flex-col bg-white dark:bg-gray-900">
        <div class="px-8 py-6 border-b border-gray-200 dark:border-gray-700">
            <PostsHeader @open-upload-modal="showUploadModal = true" />
        ...
        <div class="flex-1 overflow-y-auto p-8">
            <div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <div v-for="video in filteredposts" :key="video.id"
                    ...
                    <div class="relative aspect-video bg-gray-200 dark:bg-gray-700 overflow-hidden">
                        ...
                        <div class="absolute top-2 left-2">
                            <label class="custom-checkbox">
                                <input type="checkbox" :checked="selectedposts.has(video.id)"
                                    @change="togglepostselection(video.id)" />
                                <span class="checkmark"></span>
                            </label>
                        </div>
                    </div>
                    <div class="p-4">
                        <h3 class="font-semibold text-gray-900 dark:text-gray-100 mb-2 truncate cursor-pointer hover:text-red-600 transition-colors"
            ...
            <div v-else
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            <th class="px-6 py-3 text-left">
                                <label class="custom-checkbox">
                                    <input type="checkbox" :checked="selectedposts.size === filteredposts.length"
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <tr v-for="video in filteredposts" :key="video.id"
                                    ...
                                    <div class="min-w-0">
                                        <div class="font-medium text-gray-900 dark:text-gray-100 truncate cursor-pointer hover:text-red-600"
                                            @click="router.push({ name: 'video', params: { id: video.id } })">{{
                                                video.title }}</div>
                                        <div v-if="video.status === 'uploading' && video.progress !== undefined"
                                            class="mt-1">
                                            <div
            ...
            <div v-if="filteredposts.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">No posts found</h3>
        ...
        <div v-if="showUploadModal"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full p-8" @click.stop>
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Upload posts</h2>
                ...
                <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-12 text-center"
                    ...
                    <p class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Drop your posts here</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto"
                ...
                <div class="px-8 py-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <button @click="togglePlatform('tiktok')" type="button" :class="[
                                ...
                                <div class="text-left flex-1">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Short posts</div>
                                </div>
                        ...
                        <p v-if="videoDetailsForm.platforms.length === 0"
