...
import { fetchVideoComments } from './videoComments.js';
dotenv.config();

class CommentsWorker {
  constructor() {
    this.workerId = process.env.WORKER_ID || `comments-worker-${uuidv4()}`;
    this.workerName = process.env.WORKER_NAME || `Comments-Worker-${os.hostname()}`;
  ...
  async start() {
    console.log('🚀 Starting Comments Worker...');
    console.log('================================');
    ...
    try {
      ...
      this.isRunning = true;
      console.log('================================');
      console.log('✅ Comments Worker is now running and ready!');
  ...
  async processJob(job) {
    console.log(`\n📊 Processing comments job: ${job.job_id}`);
    console.log(`   Platform: ${job.platform}`);
